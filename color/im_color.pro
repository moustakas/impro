...
FUNCTION im_Color, theColor, colorIndex, $
   AllColors=allcolors, $
...
; jm08aug22nyu - 
   whiteindex = where(strmatch(strcompress(strlowcase(colors),/remove),'white'))
   blackindex = where(strmatch(strcompress(strlowcase(colors),/remove),'black'))
   if (!d.name eq 'X') then theindex = whiteindex else $
     theindex = blackindex
;  Message, "Can't find color " + theColor + ". Substituting " + StrUpCase(colors[theindex]) + ".", /Informational
   theColor = theNames[theindex]
