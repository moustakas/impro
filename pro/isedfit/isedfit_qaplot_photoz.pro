;+
; NAME:
;   ISEDFIT_QAPLOT_PHOTOZ
;
; PURPOSE:
;   Generate photometric redshift QAplots based on the ISEDFIT_PHOTOZ
;   output. 
;
; INPUTS:
...
pro isedfit_qaplot_photoz, isedfit_paramfile, params=params, thissfhgrid=thissfhgrid, $
  isedfit_dir=isedfit_dir, montegrids_dir=montegrids_dir, outprefix=outprefix, $
  index=index, galaxy=galaxy1, pdffile=pdffile, nrandom=nrandom, xrange=in_xrange, $
  yrange=in_yrange, xlog=xlog, isedfit_results=isedfit_results, clobber=clobber
    ...
    if n_elements(isedfit_paramfile) eq 0 and n_elements(params) eq 0 then begin
       doc_library, 'isedfit_qaplot_photoz'
       return
    endif
...
; treat each SFHgrid separately
    ...
    if ngrid gt 1 then begin
       for ii = 0, ngrid-1 do begin
          isedfit_qaplot_photoz, params=params[ii], isedfit_dir=isedfit_dir, $
            outprefix=outprefix, index=index, galaxy=galaxy1, pdffile=pdffile, $
            xrange=in_xrange, yrange=in_yrange, xlog=xlog, nrandom=nrandom, $
            clobber=clobber, isedfit_results=isedfit_results1
...
; allow the user to overwrite PDFFILE
    fp = isedfit_filepaths(params,outprefix=outprefix,isedfit_dir=isedfit_dir,$
      montegrids_dir=montegrids_dir,photoz_pdffile=pdffile)
    if file_test(fp.isedfit_dir+fp.qaplot_photoz_pdffile) and $
      keyword_set(clobber) eq 0 then begin
       splog, 'Output file '+fp.qaplot_photoz_pdffile+' exists; use /CLOBBER'
       return
    endif

; restore the photometric redshift fitting results; optionally restore
; just NRANDOM random galaxies 
...
; generate the QA-plot
    ...
    im_plotconfig, 0, pos, psfile=fp.isedfit_dir+fp.qaplot_photoz_psfile, $
...
; y-title       
    ...
    print
    im_plotconfig, psfile=fp.isedfit_dir+fp.qaplot_photoz_psfile, /psclose, /pdf
