...
; add the photometric redshift info    
    if keyword_set(photoz) then begin
       pofz = {$
         photoz:      fltarr(5),$ ; five best redshifts
         photoz_err:  fltarr(5),$ ; corresponding uncertainties
         photoz_chi2: fltarr(5),$ ; corresponding chi2 values
         z:                -1.0,$ ; maximum likelihood redshift
         z_sigma:   [-1.0,-1.0],$ ; upper and lower 1-sigma uncertainty
         z_95:      [-1.0,-1.0],$ ; upper and lower 95% CL
         z_err:          -1.0}    ; mean redshift uncertainty
...
; models/templates that violated the maxage prior
          ...
          isedfit_results[igal].photoz = zmin.photoz
          isedfit_results[igal].photoz_err = zmin.photoz_err
          isedfit_results[igal].photoz_chi2 = zmin.chi2min
          isedfit_results[igal].z = isedfit_results[igal].photoz[0]
          isedfit_results[igal].z_err = isedfit_results[igal].photoz_err[0]
          isedfit_results[igal].z_sigma = zmin[0].photoz_sigma
          isedfit_results[igal].z_95 = zmin[0].photoz_95
