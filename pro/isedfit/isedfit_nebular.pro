...
Function isedfit_nebular, nlyc, wave=wave, inst_vsigma=inst_vsigma, $
  vsigma=vsigma, oiiihb=oiiihb1, line=line
...
; line-width parameters, including instrumental velocity width     
    ...
    if n_elements(inst_vsigma) eq 0 then inst_vsigma = 0.0 ; [km/s]
...
; adopt a fiducial [OIII]/Hbeta line-ratio
    ...
    if noiiihb ne 0 then begin
       if noiiihb eq 1 then oiiihb = replicate(oiiihb1,nspec) else begin
          if noiiihb ne nspec then begin
             splog, 'Dimensions of OIIIHB and NLYC must match!'
             return, -1
          endif
       endelse
    endif else oiiihb = replicate(10.0^(-0.4),nspec)
...
; build the emission-line luminosities, starting with the
; hydrogen/helium lines; calculate the luminosity of each line (in
; erg/s/A) given N(Lyc); adopted conversions are from Kennicutt 1998
; (but see also Leitherer & Heckman 1995 and Hao+2011)
    hlineinfo = replicate({id: 0, name: '', wave: 0D, flux: 0D, ratio: 0.0},nhline)
    hlineinfo.name = hdata.name
    hlineinfo.wave = hdata.wave
    
    isha = where(hdata.name eq 'Halpha',nha)
    ishb = where(hdata.name eq 'Hbeta',nhb)
...
;   niceprint, hdata.name, hdata[isha].emissivity/hdata.emissivity
    ...
    for ii = 0, nhline-1 do hlineinfo[ii].flux = 1.367D-12*nlyc*$ ; [erg/s/A]
...
; [OIII]/Hbeta ratio
    ...
    flineinfo.name = fdata.name
    flineinfo.wave = fdata.wave
    
    for ii = 0, nfline-1 do begin
       ratio = poly(alog10(oiiihb),fdata[ii].coeff)+randomn(seed)*fdata[ii].scatter
...
; add [OIII] 5007, with no scatter
    ...
    oiii.wave = 5006.842D
    oiii.ratio = oiiihb
    oiii.flux = hlineinfo[ishb].flux*oiiihb
...
;     ysty=3, psym=8, xr=[1,1E3], /xlog, yr=[0,2]
    ...
    this = where(flineinfo.name eq '[OII]_3727',comp=therest)
...
; add [SII] 6731
    ...
    this = where(flineinfo.name eq '[SII]_6716')
...
; add [OIII] 4959
    ...
    this = where(flineinfo.name eq '[OIII]_5007')
...
; add [NII] 6584
    ...
    this = where(flineinfo.name eq '[NII]_6584')
...
; sampling around the lines
    if n_elements(wave) eq 0 then begin
       wave = range(912.0,5E4,1000) ; [Angstrom]
       for ii = 0, nline-1 do wave = [wave,range(line[ii].wave-20,line[ii].wave+20,50)]
       wave = wave[sort(wave)]
...
; each pixel is a fixed width in velocity space (km/s)
    ...
    for jj = 0, nline-1 do begin
       lineflux += line[jj].flux*exp(-0.5*(log10wave-alog10(line[jj].wave))^2/log10sigma^2)/$
         (sqrt(2.0*!pi)*log10sigma)/alog(10)
    endfor
    flam_line = interpolate(lineflux,findex(10.0^log10wave,wave),missing=0.0)
;   djs_plot, 10^log10wave, lineflux, psym=8, xrange=[4800,5050]
;   djs_oplot, wave, flam_line, psym=8, color='green'
    
; now build the nebular continuum spectrum
    flam_cont = nlyc*interpolate(nebcont.flam_neb,findex(nebcont.wave_neb,wave)) ; [erg/s/A]
