...
function im_total_ir, iras_flux, iras_ferr=iras_ferr, alpha=alpha
...
; if there are multiple objects then call this routine recursively 
    ...
    if (ndim eq 2L) then begin
       ...
       for igalaxy = 0L, ngalaxy-1L do begin
          result1 = im_total_ir(iras_flux[*,igalaxy],iras_ferr=iras_ferr[*,igalaxy],alpha=alpha)
...
; initialize the output data structure
    ...
    result = {$
      ...
      ir_flux_short:  -1.0, $ ; 8-100 micron flux [erg/s/cm2]
      ir_ferr_short:  -1.0, $ ; error in above
      ir_flux_long:   -1.0, $ ; 100-1000 micron flux [erg/s/cm2]
      ir_ferr_long:   -1.0, $ ; error in above
      ir_flux:        -1.0, $ ; total (8-1000 micron) flux [erg/s/cm2]
      ir_ferr:        -1.0}   ; error in above
...
; initialize parameters of the IRAS satellite and some constants 
    ...
    ir_wave_extended = [8.0,12.0,25.0,60.0,100.0] 
    ir_flux_extended = [ir_flux[0],ir_flux]
    ir_ferr_extended = [ir_ferr[0],ir_ferr]
...
; integrate the flux density between 8 and 100 microns

;   result.ir_flux_short = int_tabulated(ir_wave_extended,ir_flux_extended)
;   result.ir_ferr_short = sqrt(int_tabulated(ir_wave_extended,ir_ferr_extended^2))

    result.ir_flux_short = qpint1d('qpint1d_func',ir_wave_extended[0],$
      ir_wave_extended[4],functargs={wave:ir_wave_extended,flux:ir_flux_extended})
    result.ir_ferr_short = sqrt(qpint1d('qpint1d_func',ir_wave_extended[0],$
      ir_wave_extended[4],functargs={wave:ir_wave_extended,flux:ir_ferr_extended^2}))
...
;   past_100 = where((wave ge 100.0) and (wave le 1000.0))
;   ir_flux_long_min = int_tabulated(wave[past_100],BB_min[past_100])
;   ir_flux_long_max = int_tabulated(wave[past_100],BB_max[past_100])

    ir_flux_long_min = qpint1d('qpint1d_func',100.0,1000.0,functargs={wave:wave,flux:BB_min})
    ir_flux_long_max = qpint1d('qpint1d_func',100.0,1000.0,functargs={wave:wave,flux:BB_max})

    result.ir_flux_long = (ir_flux_long_min + ir_flux_long_max) / 2.0
    result.ir_ferr_long = (ir_flux_long_min - ir_flux_long_max) / 2.0

;   result.ir_flux_long = int_tabulated(wave[past_100],BB[past_100])
;   result.ir_flux_long = tsum(wave,BB,100.0,maxwave)

    result.ir_flux = result.ir_flux_short + result.ir_flux_long
    result.ir_ferr = sqrt(result.ir_ferr_short^2 + result.ir_ferr_long^2)
