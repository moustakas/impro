;######################################################################
;
; Copyright (C) 2001-2004, Michele Cappellari
...
;
; PURPOSE:
;   Logarithmically rebin a spectrum, while rigorously conserving both the
;   total flux and the flux density. Basically the photons in the spectrum
;   are simply ridistributed according to a new grid of pixels,
;   with non-uniform size in the spectral direction.
;
...
;   NB: This rebinning of the error-spectrum is approximate as it
...
;       of the first and last pixels in the spectrum, which is assumed to
;       have constant wavelength scale! E.g. from the values in the
...
;   LOGLAM: log(lambda) (natural logarithm: ALOG) of the central wavelength of each
;       pixel. This is the log of the *geometric* mean of the borders of each pixel.
;
; KEYWORDS:
;   OVERSAMPLE: Small oversampling can be done, not to loose
;       spectral resolution and to avoid aliasing.
...
;   V2.21: Verify that lamRange[0] < lamRange[1]. MC, Vicenza, 29 December 2004
;----------------------------------------------------------------------
...
if n_params() ge 4 then $  ; Optional output log(wavelength): log of geometric mean!
    logLam = alog(sqrt((newBorders*shift(newBorders,1))[1:*])*dLam)
