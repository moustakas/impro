...
;------------------------------------------------------------------------------
pro im_struct_print, struct, filename=filename, lun=lun_in, tarray=tarray, $
...
; rsex format
         rsexhdr = strtrim(strsplit(hdr1,' ',/extract),2)
         for ii = 0L, n_elements(rsexhdr)-1L do $
           printf, lun, string(ii+1L,format='(I0)')+' '+rsexhdr[ii]
;        printf, lun, hdr1
;        printf, lun, hdr2
      endif
