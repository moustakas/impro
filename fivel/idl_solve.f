...
c
c CALLING SEQUENCE:
c     call idl_solve(den,tem,t,weight,a,c,pop,jl,ncrit,e,q)
c
c INPUTS:
c     den    - electron density [cm-3]
c     tem    - electron temperature [K]
c     t      - energy-level separation [5] (IDL_PARI)
c     weight - statistical weights for each level [5] (IDL_PARI)
c     a      - radiative transition probabilities [5,5] (IDL_PARI)
c     c      - collision strengths [5,5] (IDL_PARII)
c
c OUTPUTS:
...
c-
      ...
      subroutine idl_solve(den,tem,t,weight,a,c,pop,jl,ncrit,e,q)
...
c     
 ...
 1300    continue
         ...
         do 1400 j=2,4
            t1=a2(j,j)
            if(t1 .eq. 0.0) then
c              write(filout,*)'Divide by 0 during T1 normalization
c    1              in SOLVE'
               stop
            endif
            dtl(i)=dtl(i)*t1
