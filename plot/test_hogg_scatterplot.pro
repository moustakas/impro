...
pro im_hogg_scatterplot, xxx,yyy,weight=weight, $
                      ...
                      usegrid=usegrid, $
  overplot=overplot,reverse_grey=reverse_grey,$
...
; initialize the plot without the axes
...
if NOT keyword_set(nogreyscale) then begin
    ...
    if keyword_set(reverse_grey) then tvgrid = tvgrid-max(tvgrid)
;   if keyword_set(reverse_grey) then tvgrid = $
;     bytscl(mingrey-tvgrid,top=mingrey)
;     bytscl(mingrey-tvgrid,top=mingrey-darkest)+darkest

;   tvgrid = bytscl(grid,min=mingrid,max=maxgrid)

; plot greyscale; re-draw the axes
;   pos = [!X.Window[0], !Y.Window[0], !X.Window[1], !Y.Window[1]]
;   tv, tvgrid, pos[0]*!d.x_vsize, pos[1]*!d.y_vsize, $
;     xsize=(pos[2]-pos[0])*!d.x_vsize+10, ysize=(pos[3]-pos[1])*!d.y_vsize
;   tv, tvgrid,xrange[0],yrange[0],/data, $
;     xsize=(xrange[1]-xrange[0]),ysize=(yrange[1]-yrange[0])
    tvimage, tvgrid, /overplot
    plot, [0],[0],xrange=xrange,yrange=yrange,/xstyle,/ystyle, $
      _EXTRA=KeywordsForPlot,/nodata,/noerase ;,color=djs_icolor('default')
; hack! overplot *just* the axes, always in either black or white;
; this piece of code is if you want the axis labels to have a
; different color than the axes themselves (see, e.g.,
; MZPLOTS_LOG12OH) 
;;    if (n_elements(KeywordsForPlot) ne 0L) then begin
;;       KeywordsForPlot1 = KeywordsForPlot
;;       if tag_exist(KeywordsForPlot1,'COLOR') then KeywordsForPlot1.color = djs_icolor('default')
...
;;    endif
;;    plot, [0],[0],xrange=xrange,yrange=yrange,/xstyle,/ystyle, $
;;      _EXTRA=KeywordsForPlot1,/nodata,/noerase,xtitle='',ytitle='',$
