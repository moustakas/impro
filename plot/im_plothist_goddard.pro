PRO im_plothist, arr, xhist,yhist, BIN=bin,  NOPLOT=NoPlot, OVERPLOT=Overplot, $
  ...
  fraction=fraction, normfactor=normfactor, yoffset=yoffset
...
;      /FILL - if set, will plot a filled (rather than line)
;              histogram.
;	fraction - normalize to the total in all the bins
;
...
;-
;			Check parameters.
; jm02oct15uofa - added fraction keyword
    
 On_error,2

 if N_params() LT 1 then begin   
	print,'Syntax - im_plothist, arr, [xhist,yhist, ' 
...
; normalize to the total (fraction)

if (n_elements(yoffset) ne 0L) then yhist = yhist + yoffset

if keyword_set(fraction) then begin ; jm
   if (n_elements(normfactor) eq 0L) then normfactor = total(yhist) 
   yhist = yhist / float(normfactor)
endif
