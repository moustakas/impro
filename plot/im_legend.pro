...
pro im_legend, items, BOTTOM_LEGEND=bottom, BOX = box, CENTER_LEGEND=center, $
...
;
    ...
    if (nsth ne 0) and (nsth ne n) and (nsth NE 1) then message, $
      'Must have 0, 1 or '+strn+' elements in SYMTHICK array.'
    ...
    case nsth of 
       0: symthick = replicate(!p.thick,n) ;Default = !P.THICK
       1: symthick = intarr(n) + symthicki
       else: symthick = symthicki
    endcase 

    if n_elements(charsize) eq 0 then charsize = !p.charsize
    if charsize eq 0 then charsize = 1
    case n_elements(symsizei) of ; jm08aug25nyu
       0: symsize = charsize + intarr(n)
       1: symsize = charsize + intarr(n)
...
;
    ...
    if n_elements(number) eq 0 then number = 1.0
    case N_elements(colorsi) of 
; jm08aug21nyu
       0: colors = replicate('',n) ;Default is !P.COLOR
...
; jm08aug21nyu
       0: textcolors = replicate('',n) ;Default is !P.COLOR
...
; jm08aug21nyu
    if n_elements(outline_color) EQ 0 then outline_color = ' '
...
;
    ...
    if n_elements(pspacing) eq 0 then pspacing = 1*charsize ; 3*charsize
...
;            if (psym[i] eq 0) and (vectorfont[i] eq '') then num = number else num = number
...
;    if (num eq 1) and vertical then xp = x + xt/2      ; IF 1, CENTERED.
             ...
             if symline and (linestyle[i] ge 0) then begin
                if (psym[i] lt 0) then $
                  ppsym = -symcat(-psym[i],thick=symthick[i]) else $
                  ppsym = symcat(psym[i],thick=symthick[i])
                if (size(colors[i],/type) eq 7) then $
                  col = im_color(colors[i],101) else col = colors[i]
                plots,xp,yp,color=col,$
                  /normal,linestyle=linestyle[i],psym=ppsym, $
                  symsize=symsize[i], thick=thick[i]
             endif
          endelse
          ...
          if (size(textcolors[i],/type) eq 7) then $
            col = im_color(textcolors[i],101) else col = textcolors[i]
          xyouts,x,y,delimiter,width=width,/norm,color=col, $
...
;       =====>> CLEAR AREA
          ...
          polyfill,[x,xend,xend,x,x],y + [0,0,ywidth,ywidth,0],/norm,color=djs_icolor('white') ; color=-1
...
;
          ...
          corners = [x,y+ywidth,xend,y]
; jm08aug21nyu
          if (size(outline_color,/type) eq 7) then $
            col = im_color(outline_color,101) else col = outline_color
