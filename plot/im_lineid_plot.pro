...
;
	...
	if n_elements(extend_thick) eq 0 then extend_thick = 0
	if n_elements(extend_thick) eq 1 then ethick = replicate(extend_thick,n) $
				   else ethick = extend_thick
	...
	if n_elements(cthick) eq 1 then cthick = replicate(cthick,n)
        ...
        case n_elements(label_color) of
           0L: label_color1 = replicate(djs_icolor('default'),n)
           1L: label_color1 = replicate(label_color,n)
           else: label_color1 = label_color
        endcase

        case n_elements(extend_color) of
           0L: extend_color1 = replicate(djs_icolor('default'),n)
           1L: extend_color1 = replicate(extend_color,n)
           else: extend_color1 = extend_color
        endcase
;
...
;       
        ...
        plot,[0], [0], /nodata,_EXTRA=extra,pos = pos, color=axis_color
        oplot,wave,flux,_EXTRA=extra,color=spectrum_color
...
print_text:
        ...
        stop_arrow = ymax + (yrange/30)*3.0 ; 4
        ...
        for i=0,nlines-1 do begin
           plots,[wl[i],wl[i],wlp[i],wlp[i]], thick=ethick[i], $
             [start_arrow,bend1,bend2,stop_arrow], color=extend_color1[i]
           xyouts,wlp[i] + char_height[i]/2, start_text1, txt1[i], $
             orientation = 90, size=csize[i], charthick = cthick[i], color=label_color1[i]
...
;
	...
	for i=0,n-1 do plots,[ww[i],ww[i]],[(ff[i]+offset)<ymax>ymin,ymax], $
          line=2,thick = ethick[i], color=extend_color1[i]
