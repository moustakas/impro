...
;; Main program
pro im_plotimage, img0, xrange=xrange0, yrange=yrange0, $
               ...
               pixtolerance=pixtolerance, _EXTRA=extra, negative=negative
  ...
  if nimgdims EQ 2 then begin
      ...
      if imgtype NE 1 then begin
          ...
          img = bytscl(img, min=range(0), max=range(1), top=ncolors-1B) $
            + bottom
          if keyword_set(negative) then img = byte(ncolors-1B) - temporary(img) ; jm07jan16nyu
      endif
