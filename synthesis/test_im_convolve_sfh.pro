...
; read the SSP and test tau-model
    minwave = 3000.0 & maxwave = 9000.0
    ssp = im_read_bc03(minwave=minwave,maxwave=maxwave,bc03_extras=essp)

; --------------------------------------------------
; test - compare against BC03 and make a QAplot
    infosfh = {tau: 5.0, nburst: 0, ebv: 0.0}
;   time = ssp.age/1D9
;   outflux = isedfit_convolve_sfh(ssp,infosfh=infosfh,time=time,mstar=essp.m_,cspmstar=cspmstar)
;   sfh = isedfit_reconstruct_sfh(infosfh,age=time)
    outflux = im_convolve_sfh(ssp,tau=infosfh.tau,sfh=sfh,time=time,mstar=essp.m_,cspmstar=cspmstar)

    tau = im_read_bc03(isedfile='tau5.ised',$
