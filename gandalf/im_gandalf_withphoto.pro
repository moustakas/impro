...
PRO SET_CONSTRAINTS, GALAXY=galaxy, NOISE=noise, CSTAR=cstar,                                     $
                     ...
                     FOR_ERRORS=for_errors, $
  modelmaggies=modelmaggies, maggies=maggies, ivarmaggies=ivarmaggies, filterlist=filterlist
splog, 'here!'
...
FUNCTION FITFUNC_GAS, pars, CSTAR=cstar, GALAXY=galaxy, NOISE=noise, EMISSION_SETUP=emission_setup, $
                      ...
                      INT_DISP=int_disp, LOG10=log10, REDDENING=reddening, L0_TEMPL=l0_templ,       $
                      FOR_ERRORS=for_errors, $
  modelmaggies=modelmaggies, maggies=maggies, ivarmaggies=ivarmaggies
compile_opt idl2, hidden
...
goodmaggies = lindgen(nmaggies)+npix
...
IF s[0] EQ 2 THEN ntemp = s[2] ELSE ntemp = 1   ; Number of template spectra
splog, 'Here!'
c = dblarr(npix+nmaggies,degree+nlines+ntemp+1,/NOZERO) ; This array is used for estimating predictions
...
splog, 'here!'
FOR j=0,degree   DO c[0:npix-1,j] = legendre(x,j) ; Legendre polinomials (additive)
;FOR j=0,degree   DO c[0:npix-1,j] = legendre(x,j)      ; Legendre polinomials (additive)
IF NOT KEYWORD_SET(reddening) THEN BEGIN
splog, 'here!'
    FOR j=0,ntemp-1  DO c[0:npix-1,degree+1+j] = mpoly*cstar[0:npix-1,j] ; Convolved templates x mult. polinomials
    FOR j=0,nlines-1 DO c[0:npix-1,degree+ntemp+1+j]=gaus[*,j]           ; Emission lines
...
splog, 'include the model photometry'
    FOR j=0,ntemp-1  DO c[npix:npix+nmaggies-1,degree+1+j] = modelmaggies[*,j]
 ENDIF ELSE BEGIN
    ; redden both stellar and emission-line templates
splog, 'No do not tie the reddening together!!'
...
ENDELSE
 ...
 splog, 'here!'
FOR j=0,degree+ntemp+nlines DO a[0:npix-1,j] = c[0:npix-1,j]/noise          ; Weight all columns with errors
FOR j=0,ntemp-1 DO a[npix:npix+nmaggies-1,degree+1+j] = c[npix:npix+nmaggies-1,degree+1+j]*sqrt(ivarmaggies)
...
splog, 'here!'
themodel = a[[goodpixels,goodmaggies],*]
thegalaxy = [galaxy[goodpixels],maggies]*[1.0/noise[goodpixels],sqrt(ivarmaggies)]
sol = BVLSN_Solve_pxf(themodel,thegalaxy,degree, $
                      FOR_ERRORS=for_errors,NLINES=nlines)
;sol = BVLSN_Solve_pxf(a[goodpixels,*],galaxy[goodpixels]/noise[goodpixels],degree, $
;                      FOR_ERRORS=for_errors,NLINES=nlines)
bestfit = c # sol
splog, 'here!'
err = ([galaxy[goodpixels],maggies]-bestfit[[goodpixels,goodmaggies]])*[1.0/noise[goodpixels],sqrt(ivarmaggies)]
...
PRO im_GANDALF, templates, galaxy, noise, velScale, sol, emission_setup, l0_gal, lstep_gal,       $
              ...
              BESTFIT=bestFit, EMISSION_TEMPLATES=emission_templates, WEIGHTS=weights, ERROR=esol, $
              PLOT=plot, QUIET=quiet, LOG10=log10, REDDENING=reddening, L0_TEMPL=l0_templ,         $
              FOR_ERRORS=for_errors, modelmaggies=modelmaggies, maggies=maggies, ivarmaggies=ivarmaggies, filterlist=filterlist, $
  photofit=photofit
compile_opt idl2
on_error, 2
...
window, 0
djs_plot, galaxy
djs_oplot, bestfit[0:3962], color='red'
weff = k_lambda_eff(filterlist=filterlist)
window, 2
djs_plot, weff, maggies, psym=6, symsize=6, xsty=3
djs_oplot, weff, bestfit[3963:3967], psym=6, symsize=6, color='red'
print, total(ivarmaggies*(maggies-bestfit[3963:3967])^2)
stop
; ------------------------------------
