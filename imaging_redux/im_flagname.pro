...
;
; CALLING SEQUENCE:
;   label = im_flagname(flagprefix, flagvalue, [ /concat, /silent ] )
;
; INPUTS:
...
;-
;------------------------------------------------------------------------------
function im_flagname, flagprefix, flagvalue, concat=concat, silent=silent
...
;  common com_im_maskbits, maskbits
   ...
   if (n_params() NE 2 OR n_elements(flagprefix) NE 1) then begin
      print, 'Syntax - label = im_flagname(flagprefix, flagvalue, [ /concat ] )'
      return, ''
   endif
   ...
   ; (After that, store this info in a common block.)
    ...
    maskfile = filepath('im_maskbits.par',root_dir=getenv('IMPRO_DIR'),$
