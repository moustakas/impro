...
;   jm06nov07nyu - added BAD optional input
;-

pro im_avgclip,arr,average,scale=scale,NORMALIZE=normalize,SILENT=silent, $
       THRESH=thresh,bad=bad
...
if (n_elements(bad) eq 0L) then bad = byte(arr*0.0)
...
if prescale then begin
   ...
   for i=0,nimg-1 do begin
      robomean,arr[x1:x2,y1:y2,i],3.0,0.5,meanval,dummy,sigma,bad=bad[x1:x2,y1:y2,i] ; jm06nov07nyu
      means[i] = meanval
...
for i=0,nimg-1 do begin
   new = arr[*,*,i]
   resid = (new-avg)/(sigma + (sigma eq 0.0))
